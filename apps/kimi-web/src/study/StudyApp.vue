...
<!-- Kimi Study product shell. Every screen renders against the product facade
     ...
     models, permissions, or chat state. The old in-memory demo survives as a
     dev-only fixture: run the dev server and open ?study-demo=1. -->
...
const fixtureDemo = import.meta.env.DEV
  && typeof window !== 'undefined'
  && new URLSearchParams(window.location.search).has('study-demo');
const StudyDemoApp = fixtureDemo
  ? defineAsyncComponent(() => import('./StudyDemoApp.vue'))
  : undefined;
...
<template>
  <StudyDemoApp v-if="fixtureDemo && StudyDemoApp" />
  <div v-else class="study-app" :data-screen="model.screen">
