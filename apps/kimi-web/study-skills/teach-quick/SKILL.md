---
name: teach-quick
description: "[contract:teach-quick-v1] Survey an uploaded learning source end to end, clarify the learner's Mission with at most one question, and build the fastest honest course path. Use for Kimi Study quick mode when the learner is still deciding how deeply to study a book, document, transcript, or mixed set of learning materials."
---
...
Create or update `source/STUDY-SNAPSHOT.json` with mode `quick`, Skill pin `teach-quick-v1`, source status `surveying`, Mission status `not_started` or `interviewing`, and plan/generation status `not_started`.
...
Publish lessons incrementally as `lessons/NNNN-dash-case-name.html`. After each published lesson, update `publishedLessons` in the snapshot and set generation status `partially_ready`. Set it to `ready` only when every planned lesson and reference destination exists.
