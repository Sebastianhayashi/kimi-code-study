...
SPEC = importlib.util.spec_from_file_location("quick_lesson_revision", MODULE_PATH)
REVISION = importlib.util.module_from_spec(SPEC)
assert SPEC and SPEC.loader
SPEC.loader.exec_module(REVISION)

FIXTURE_PATH = Path(__file__).resolve().parent / "test_quick_gate.py"
FIXTURE_SPEC = importlib.util.spec_from_file_location("quick_gate_fixture", FIXTURE_PATH)
...
class LessonRevisionTest(unittest.TestCase):
    ...
    def test_changed_title_is_refused(self) -> None:
        with self.assertRaisesRegex(ValueError, "title"):
            self.publish(FIXTURE.quality_lesson_html(marker="另一节课"))
        self.assertEqual(self.lesson.read_text(encoding="utf-8"), self.original)
