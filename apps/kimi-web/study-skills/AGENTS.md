...
- `teach-quick` advertises `[contract:teach-quick-v2]` and writes survey evidence only.
- `teach-ria` advertises `[contract:teach-ria-v2]` and preserves all whole-reading/RIA fidelity gates.
...
Run the complete contract suite with `python3 -m pytest -q .`, or the focused commands below:
...
```bash
python3 install-study-skills.py --install --destination "${AGENT_SKILLS_DIR:?Set AGENT_SKILLS_DIR}"
python3 install-study-skills.py --check --destination "${AGENT_SKILLS_DIR}"
```
